...
## المهمة: معالجة ومنع التكرار (Idempotency Tracking by RequestId)
...
- المهمة الحالية: معالجة ومنع التكرار (Idempotency Tracking by RequestId)
- الهدف الأساسي: منع تسجيل الطالب مرتين وضمان معالجة كل RequestId لمرة واحدة فقط وإرجاع نفس النتيجة عند الإعادة
...
==================================================
- بناء ذاكرة التكرار IdempotencyCache والتحقق من بصمة الطلب ومعرف الطالب
- البنية المطلوبة للملفات: mobile_app/lib/attendance_queue/idempotency/ (idempotency_cache.dart)

==================================================
5. التقنيات وقواعد التسميات والمعايير (STANDARDS)
==================================================
- التقنيات المستخدمة: LRU In-Memory Cache, Set & Hash Lookups
- مبرر اختيار التقنيات: ضمان عدم تكرار تسجيل الحضور نهائياً حتى مع ضغط زر الإرسال عدة مرات
- قواعد التسميات المعتمدة: IdempotencyCache, IdempotencyRecord
